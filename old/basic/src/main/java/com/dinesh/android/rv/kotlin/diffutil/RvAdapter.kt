...
class RvAdapter() : RecyclerView.Adapter<RvAdapter.MyViewHolder>() {
    ...
    // New method to update the adapter data using DiffUtil
    fun updateData(newList: List<RvModel>) {
        val diffResult = DiffUtil.calculateDiff(RvDiffCallback(rvModelList, newList))
        rvModelList = newList
